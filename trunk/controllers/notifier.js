...
/**
 * @class notification.
 * handles all desctop notifications.
 * @constructor notification(<code>{options}<code>)<bre>merges the object of options of the notification contans:
 *  <p>HTMLNotificationURL:</p> not necessary if it's used to fire webkit notification,
 *  <p>defaultIcon:</p> the icon that is set to the notification in case no icon sent in <code>fireNotification</code> method,
 *  <p>checkTimeperiod:</p> the periodic time in seconds the notification seeks for items to notify for, default is 60 seconds.
 *  <p>notifySeeker:</p> the function which is to search for items for the notification.
 *  <p>localStorageObject:</p> localStorage object name, which default <code> notifySeeker </code> search for to fire notifications,
